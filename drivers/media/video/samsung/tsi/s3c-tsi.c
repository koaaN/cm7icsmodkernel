...
#include <mach/irqs.h>
#include <mach/gpio.h>
#if defined(CONFIG_CPU_S5PV210) || defined(CONFIG_LATIN_ARIES_TV)
#include <mach/map.h>
...
typedef struct  
{
	...
	int tsi_irq;
	int running;
#if defined(CONFIG_PM) && defined(CONFIG_LATIN_ARIES_TV)	//latin_cam:kihyung.nam, power management operation added
...
void s3c_tsi_set_gpio(void)
{
	...
	for(i=2;i< 7; i++)
	{
		...
		#elif !defined(CONFIG_CPU_S5PV210)  //temporarily disable gpio settings for rev0.2 P1 board,
...
#endif  //defined(CONFIG_CPU_S5PV210) || defined(CONFIG_LATIN_ARIES_TV)
...
#if defined(CONFIG_CPU_S5PV210) || defined(CONFIG_LATIN_ARIES_TV) // Fix the TSI data problem (Don't generated waking up sleep state)
...
#endif  //defined(CONFIG_PM) && defined(CONFIG_LATIN_ARIES_TV)
